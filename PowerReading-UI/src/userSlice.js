...
const getById = createAsyncThunk("meters/getById", async (id, thunkAPI) => {
  try {
...
export const addReading = createAsyncThunk(
  "meters/addReading",
  async ({ mid, date, time, kwh, pf }, { rejectWithValue }) => {
    try {
      const response = await userApi.create.addReading({
        mid,
        date,
        time,
        kwh,
